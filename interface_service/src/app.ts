...
/* importando o modelo */
import { Projeto, Administrador } from "./models/model";
...
app.get("/listAdm", listAdmHandler);
...
async function listAdmHandler(req, res) {
  console.log("Requisição de listagem adm recebida.");
  let adm = await service.listAdm();
  let json_adm_list = JSON.stringify(adm);
  res.setHeader("Content-Type", "application/json");
  res.end(json_adm_list);
}
...
async function addProjectHandler(req, res) {
  ...
  await service.insertProject(novo_projeto);
